    ...
    for img in inbox:
        ...
        if img_path.endswith(extensions):
            ...
            for monster in monster_list:
                ...
                monster_path = f"./images/{monster}.png"
...
if ("APP_INPUT_DIR" not in globals()):
    os.environ["APP_INPUT_DIR"] = "data/in"
if ("APP_OUTPUT_DIR" not in globals()):
    os.environ["APP_OUTPUT_DIR"] = "data/out"
if ("APP_CONFIG" not in globals()):
    os.environ["APP_CONFIG"] = "data/config.json"

try:
    input_path = Path(os.environ["APP_INPUT_DIR"]).resolve()
    output_path = Path(os.environ["APP_OUTPUT_DIR"]).resolve()
    config_path = Path(os.environ["APP_CONFIG"]).resolve()
